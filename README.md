...
安装脚本会自动识别当前 CPU 架构，并下载对应的 DDNS-GO Release。
...
更新过程只替换 DDNS-GO 程序本体：

```text
/usr/local/bin/ddns-go
```

**不会主动修改 DDNS-GO 原有配置文件。**
...
同时会询问是否删除 Installer 创建的历史程序备份：

```text
/usr/local/share/ddns-go-installer
```

如果希望以后重新安装 DDNS-GO，建议保留原有配置。
